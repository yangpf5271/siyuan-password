# 思源笔记密码锁功能 - 技术实现细节补充文档

**版本**: 1.0
**更新日期**: 2025-12-31
**基于**: v2.1 设计文档
**目的**: 补充原设计方案中省略的技术实现细节

---

## 📋 文档说明

本文档旨在补充《笔记加锁功能设计文档 v2.1》中的技术细节,提供完整的代码实现指南和架构说明。所有内容基于思源笔记源码验证,确保可行性。
...
基于 `app/src/plugin/index.ts` 的 Plugin 基类,密码锁插件的完整入口实现:
...
export default class PasswordLockPlugin extends Plugin {
    ...
    async onload() {
        ...
        // 5. 注册 WebSocket 事件处理器 (通过 EventBus)
...
使用插件数据持久化 API (`Plugin.loadData/saveData`) 实现 SQLite 数据库:
...
export interface PasswordLockRecord {
    ...
    created_at?: string;
    updated_at?: string;
    updated_by?: string;            // 设备ID或用户ID
    failed_attempts: number;
    locked_until?: string;          // ISO 时间字符串
...
import bcrypt from "bcryptjs";  // 需要在 package.json 添加依赖
        ...
        if (lock.locked_until) {
            ...
            if (now < lockedUntil) {
                ...
                return {
                    success: false,
                    error: "LOCKED",
                    message: `密码验证已锁定,请等待 ${remainingSeconds} 秒`,
...
### 2.1 openFileById Hook 拦截器

基于源码 `app/src/editor/util.ts:39` 中的 `openFileById` 函数:
...
export class DocumentOpenInterceptor {
    ...
    uninstall(): void {
        ...
        console.log("DocumentOpenInterceptor uninstalled ✅");
    }
...
基于 `app/src/index.ts:60-163` 中的 WebSocket 消息处理:
...
export class WebSocketEventHandler {
    ...
    register(): void {
        ...
        console.log("WebSocketEventHandler registered ✅");
    }
    ...
    unregister(): void {
        ...
        // EventBus 的 off 方法需要相同的函数引用
        // 如果 EventBus 不支持 off,则无法完全注销
        // 这里标记为未注册,handleWebSocketMessage 中添加检查

        this.registered = false;
        console.log("WebSocketEventHandler unregistered ✅");
    }
...
基于源码验证,`/data/storage/petal/` 路径会被思源云同步自动处理:

**文件路径**: `/data/storage/petal/siyuan-password/password_locks.db`
...
Plugin.saveData("password_locks.db", data)
  ↓
API调用: /api/file/putFile
  ↓
思源检测到 /storage/petal/ 路径变化
  ↓
repository.go 处理云同步
  ├─ Upserts → 上传到云端
  └─ Removes → 删除标记
...
设备B收到同步事件
  ↓
WebSocket事件: "reloadPlugin"
  ↓
Plugin.onload() 重新执行
  ↓
DatabaseService.initialize() 加载新数据
  ↓
✅ 同步完成
```

**冲突解决策略**: Last-Write-Wins (最后写入胜出)

```typescript
// src/sync-handler.ts (可选,用于处理同步冲突)
...
| 场景 | 处理方式 |
|------|---------|
| **文档在验证密码前被删除** | `interceptOpen` 中捕获异常,返回 false |
| **WebSocket 连接断开** | Plugin 自动重连,事件不丢失 |
| **数据库文件损坏** | `initialize()` 捕获异常,创建备份并重建 |
| **并发密码验证** | bcrypt 是线程安全的,WeakMap 原子操作 |
| **多设备同时修改密码** | Last-Write-Wins,后修改的覆盖 |
| **插件卸载后重装** | 数据库文件保留,`loadData` 恢复状态 |
| **用户忘记密码** | 使用恢复密钥重置,或手动删除 db 文件 |
...
1. **数据库查询优化**:
   - 使用内存索引 (Map) 缓存热点数据
   - 减少 `saveDatabase()` 调用频率 (批量更新)

2. **密码验证优化**:
   - bcrypt 成本因子 10 (平衡安全和性能)
   - WeakMap 查询 O(1) 时间复杂度

3. **UI 渲染优化**:
   - 密码对话框使用原生 DOM,避免大型框架
   - 事件委托减少监听器数量
...
- [x] 密码使用 bcrypt 加密 (不可逆)
- [x] 防暴力破解 (指数退避延迟)
- [x] WeakMap 防止内存泄露和外部访问
- [x] 恢复密钥加密存储
- [x] 审计日志完整记录
- [x] 敏感数据不存储在全局对象
- [x] Hook 函数安全卸载 (防止内存泄露)
- [x] 错误处理不泄露敏感信息
...
**新增章节**:
1. **第7章: 技术实现细节** (本文档内容)
2. **第8章: API集成说明** (基于思源笔记API分析报告)
3. **第9章: 测试策略** (单元测试+集成测试)
4. **第10章: 部署和配置** (插件安装、配置说明)

**修改章节**:
1. **第4.4节**: 补充 openFileById Hook 完整代码
2. **第4.5节**: 补充云同步详细流程
3. **第5章**: 添加错误处理和边缘情况
4. **第6章**: 更新开发计划的技术栈依赖
...
```
Week 1-2: Phase 0 - 核心功能
  ├─ Day 1-2: 数据库服务实现
  ├─ Day 3-4: 密码管理器实现
  ├─ Day 5-6: openFileById Hook 实现
  ├─ Day 7-8: WebSocket 事件处理
  └─ Day 9-10: 集成测试

Week 3-4: Phase 1 - UI 和功能完善
  ├─ Day 1-3: 密码对话框 UI
  ├─ Day 4-6: 设置界面
  ├─ Day 7-8: 审计日志查看
  └─ Day 9-10: 恢复密钥管理

Week 5-6: Phase 2 - 质量保证
  ├─ Day 1-3: 单元测试覆盖率 >80%
  ├─ Day 4-6: 集成测试和边缘用例
  ├─ Day 7-8: 性能测试和优化
  └─ Day 9-10: 安全审计和文档完善

Week 7: Phase 3 - Beta 测试
  ├─ Day 1-2: 内部测试
  ├─ Day 3-4: Bug 修复
  └─ Day 5-7: 用户文档和发布准备
